...
fn crawl(url: &str) {
    ...
    unsafe { VURLS.push(_clean_url.clone()) };
    add_url(&mut unsafe { FILE_PATH.clone().as_str() }, &_clean_url.clone());
    // if !res
    //     .headers()
    //     .get("content-type")
    //     .unwrap()
    ...
    for link in document.select(&links) {
        ...
        if !unsafe { VURLS.contains(&_clean_link) } && !unsafe { UURLS.contains(&_clean_link) } {
    ...
    loop {
        if unsafe { UURLS.len() } > 0 {
            ...
            if unsafe { VURLS.contains(&url) } {
                ...
                print_status(&url);
                let crawlurl = url.clone();
                // let other_handler = std::thread::spawn(move || {
                // });
                // other_handler.join().expect("Unable to join thread");
